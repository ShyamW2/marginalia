...
- **M39 §A0 — `page.render()`/`@napi-rs/canvas` confirmed on Linux only.** This session ran
  entirely on the Linux (Node 24) machine; PDF.md §A0 asks it confirmed "on both machines"
  per [[marginalia-two-machine-setup]] before §A5/§A6 (rasterization) are trusted. Needs a
...
## M39 §A — the extractor, and what the §A8 gate actually found — 2026-09-03

Implemented `server/src/library/pdf/` (`headerFooter.ts`, `columns.ts`, `lines.ts`,
`equations.ts`, `figures.ts`, `blocks.ts`, `rasterize.ts`, `extract.ts`), each heuristic
from PDF.md §3 as its own pure, independently-tested module, orchestrated by `extractPdf`.
32 unit/integration tests, `pnpm test` green.

**A0.** `pdfjs-dist` + `@napi-rs/canvas` confirmed live on this machine (Linux, Node 24):
`getTextContent()` returns items carrying `transform`/`width`/`height`/`fontName` as
PDF.md §3 assumes, and `page.render()` through `@napi-rs/canvas` produced a real PNG with
no node-gyp step. **Not confirmed on the Mac (Node 20)** — this session only has the Linux
machine; per [[marginalia-two-machine-setup]], that check is still owed there.

**§A8, the blocking gate — run for real, with a caveat.** No real PDFs were available in
this environment (no filesystem corpus, no fetch source), so — with the operator's
explicit sign-off on this substitution before starting — the gate ran against five
synthetic-but-structurally-real PDFs built with `pdfkit`, covering the five shapes:
two-column-with-figures-and-equations, single-column preprint, report-with-tables,
book-with-outline, and an image-only scan. **This is not the real gate.** A synthetic
fixture is generated by the same person judging its output, which is exactly the blind
spot the gate exists to avoid. The outputs were still read carefully, in full, and the
exercise caught two real, generalizable bugs — recorded below — which is itself evidence
the exercise was worth doing even in its weakened form. **The real gate — five actual PDFs,
read by the operator — is still owed before this ships**, and is more likely to surface
extraction failures a synthetic fixture wouldn't think to construct (real fonts, real
column irregularities, real scanned-image noise).

**Bug 1 (fixed): a two-column page's second column fragmented into one paragraph per
line.** `linesToText`'s paragraph-break "indent" check compared every line's left edge
against a single *global* modal left edge computed across the whole page. A two-column
page has two legitimate left edges; every line in column 2 read as "indented" relative to
column 1's modal edge, so every line broke into its own paragraph — silent, and it read
fine line-by-line, which is exactly the "reads fine in a diff" failure PDF.md's gate
rationale warns about. Fixed by splitting the line stream into segments at backward
y-jumps (`splitAtBackwardJumps`) — which already exactly mark column/band boundaries, by
construction from `columns.ts` — before computing a *local* modal left edge and line
spacing per segment (`paragraphsInSegment`). Regression coverage: the existing
`lines.test.ts` cases still pass; the gate's own two-column fixture is the integration
check, re-run clean after the fix.

**Bug 2 (fixed): outline destination y-coordinates read as null for a real destination
type.** `resolveOutlineEntry` assumed every destination array had the `/XYZ` shape
(`[page, /XYZ, left, top, zoom]`, top at index 2) — but a PDF destination's shape depends
on its type (PDF 32000-1 §12.3.2.2), and pdfkit's own `outline.addItem()` emits `/Fit`
(`[page, /Fit]`, no coordinate at all). Every one of the book fixture's three chapter
entries read `y: null`, which is the exact outcome PDF.md §4 warns is destructive ("a
section that begins one-third down a page must split that page's text at the heading, not
round to the page boundary"). `destinationY()` now switches on the destination's type
name — `/XYZ`/`/FitH`/`/FitBH` carry top at index 2, `/FitR`'s is its 4th coordinate,
`/Fit`/`/FitB`/`/FitV`/`/FitBV` carry none (an honest null, not a bug). **Left open:**
`/XYZ` is what LaTeX's hyperref and most PDF-export paths emit, so a real paper's outline
is more likely to resolve than pdfkit's own — but this is inferred, not measured against a
real corpus, and is exactly the kind of thing the real gate would confirm or correct.

**Accepted, not fixed — findings that turned out to be fixture artifacts or in-scope
degradation, not bugs:**
- An early, sparser draft of the two-column fixture (one short paragraph per column)
  tripped the §3.2 "≥25% of items per mode" check into declaring a two-column page single-
  column, because a title/abstract's shorter wrapped lines (under the 70%-page-width
  full-width cutoff, so not excluded from the column histogram) were a large enough
  fraction of a deliberately tiny page to swing the ratio. Re-tested against a
  realistically dense fixture (a full column's worth of body text per column, as a real
  paper has) and the two-column case resolved cleanly with no change to `columns.ts`. Kept
  as a note rather than a code change: the failure mode is real in principle (a very short
  page could still trip it) but PDF.md's own philosophy — "a wrongly-declared two-column
  page is far more destructive than a missed one" — argues for leaving the threshold as
  specified rather than loosening it speculatively.
- A single-font synthetic "equation" (one `Helvetica-Oblique` run of glyphs, no font
  variation) correctly failed §3.4's "3+ distinct fontNames" test and stayed as ordinary
  text. This is right per spec, not a gap — a real display equation mixes math-italic,
  symbol, and roman fonts, which the unit tests in `equations.test.ts` exercise directly;
  the gate fixture just wasn't built to trigger it, and PDF.md §3.4 explicitly accepts
  inline/undetected math staying as extracted text.
- A genuinely textual table (its rows real extracted text, not an image) still matches the
  `Table\s*\d+` caption regex and can trigger the figure-region heuristic, rasterizing some
  nearby blank area as a spurious `<figure>` image even though the table's own data is
  never removed from `resource_text` (figures.ts only ever *adds* an image block; it never
  deletes a line block). Cosmetically wasteful — an empty or near-empty embedded image in
  the generated EPUB — but not a text-substrate correctness issue. Worth revisiting in M39
  §B/§C if it reads as visually noisy once real papers are through it; not fixed now
  because there is nothing in PDF.md distinguishing "table with real text rows" from
  "table that is actually a raster image" at the geometry level available to this
  heuristic.
- A 2-page test document never triggers header/footer stripping (`MIN_REPEAT_PAGES = 3`,
  correctly per PDF.md §3.1's literal "3 or more pages" — nothing to fix, just a fixture
  limitation worth knowing about when reading the other dumps: the two-column and report
  fixtures are both 2 pages, so their running footers appear untouched in the text dumps,
  while the 3-page preprint fixture shows stripping working correctly).

All five synthetic PDFs, their extracted-text dumps, and the debug scripts used to trace
both bugs above lived in the session's scratchpad, not the repo — nothing PDF-fixture-
